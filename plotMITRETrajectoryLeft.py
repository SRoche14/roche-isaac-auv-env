#!/usr/bin/env python3
"""
Plot MITRE left trajectories with Umeyama alignment.
...
Start times:
- startingLeftTimes.json provides start times (seconds). Use keys
  qualyLeft{qualy_id} for Qualisys JSON (e.g., qualyLeft6) and mitreLeft{run_id}
  for odometry/CFD/Cuboid (e.g., mitreLeft1).
- Ground truth sampling: 100 Hz. Odometry: 50 Hz. CFD/Cuboid: 50 Hz (dt = 1/50).
- Ground truth is smoothed with a robust Kalman filter + RTS smoother; filtered
  path matches raw GT length to preserve timing.

Run mapping notes:
- Qualisys files are numbered 0006/0008/0009/0010 and map to mitreLeft runs
  1/3/4/5 respectively. Test0007 / mitreLeft2 is intentionally skipped.
"""
...
START_TIMES_JSON = BASE_DIR / "startingLeftTimes.json"
...
# Map MITRE Left run IDs (bag numbering) to Qualisys test IDs.
# Test0007 / mitreLeft2 is intentionally skipped.
RUN_ID_TO_QUALY_ID = {
    1: 6,
    3: 8,
    4: 9,
...
def load_start_times(prefix: str = "qualyLeft") -> Dict[int, float]:
    """Return start times keyed by integer suffix using entries like '<prefix>{i}' in JSON."""
...
def resolve_qualy_id(run_id: int) -> int:
    """Return the Qualisys test ID for a given run_id; raise if the run is unavailable."""
    try:
        return RUN_ID_TO_QUALY_ID[run_id]
    except KeyError as exc:
        valid = ", ".join(str(r) for r in AVAILABLE_RUN_IDS)
        raise ValueError(f"Run {run_id} is not available for MITRE Left (valid runs: {valid}).") from exc
...
def spike_smoothing(
    ...
    # Estimate measurement variance from MAD of steps
    ...
    # Forward pass (robust Kalman filter)
    for k in range(n):
        if k > 0:
            x = F @ x
            P = F @ P @ F.T + Q
...
def load_and_align_run(run_id: int, qualy_starts=None, mitre_starts=None) -> Dict[str, np.ndarray]:
    ...
    if qualy_starts is None:
        qualy_starts = load_start_times(prefix="qualyLeft")
    if mitre_starts is None:
        mitre_starts = load_start_times(prefix="mitreLeft")
    start_gt = qualy_starts.get(qualy_id, qualy_starts.get(run_id, 0.0))
    start_model = mitre_starts.get(run_id, 0.0)

    gt_path = BASE_DIR / f"Curee Test{qualy_id:04d}.json"
    odom_path = pick_path("odomMITRELeft", run_id, ".csv")
    cfd_path = pick_path("positionsMITRELeftCFD", run_id, ".csv")
    cuboid_path = pick_path("positionsMITRELeftCuboid", run_id, ".csv")
    ...
    truth_raw = trim_by_time(truth_full, 100.0, start_gt)
    truth_raw = limit_by_time(truth_raw, 100.0, plot_window)
    truth = truth_raw
    truth_smooth = spike_smoothing(truth_raw)
...
# ----------------------------
def main():
    # note: 3 seconds trimmed from start of Left 1 due to poor ability to fit any data (odometry, cuboid, and cfd) to starting segment
    parser = argparse.ArgumentParser(description="Plot MITRE Left trajectories with Umeyama alignment.")
    parser.add_argument(
        "--run-id",
        type=int,
        default=1,
    ...
    # Aggregate metrics across runs for box plots
