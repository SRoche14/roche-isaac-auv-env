#!/usr/bin/env python3
"""
Plot MITRE forward trajectories with Umeyama alignment.

Generated plots per run_id:
1) Ground truth vs odometry (aligned)
2) Ground truth with Cuboid and CFD (aligned)
3) Same as #2 after removing first 2 seconds
4) Ground truth (Kalman+RTS-smoothed) with Cuboid and CFD (aligned)
...
Start times:
- startingForwardTimes.json provides a start time (seconds) per run (1-5) under keys
  qualyForward{i}; these are used for trimming ground-truth-aligned trajectories.
- Ground truth sampling: 100 Hz.
- Odometry: 50 Hz.
- CFD/Cuboid: 50 Hz (dt = 1/50).

Modifications:
qualyFoward3 had an original start time of "qualyForward3": 5.910. 
This time is calculated based on movement in the x or y axis. However, there appears to be a jump in space that occurs
between 6.449 and 6.500 s, representing a discontinuity in the trajectory likely due to tracking loss. Therefore, the starting
time is modified to be 6.500 s for better alignment.
"""
...
# this function is used in the event 'CUREE - 1' is not used, so we fallback to 1 of the markers. We choose the longest length marker due to 
# capture potentially dropping out
...
# extract xyz positions from marker parts
...
def load_start_times(prefix: str = "qualyForward") -> Dict[int, float]:
    """Return start times keyed by run_id using entries like '<prefix>{i}' in JSON."""
    # the i value is what is used in the 'out' dictionary. Most of the time i starts at 1
    # except in Up i starts from 2 and goes to 6 (this is due to an issue with a run during the data collection process on 12/16/25).
    # the first data collection process was on 12/16/25. More data collection is set for 1/16/2026.
...
def ground_truth_duration_seconds(run_id: int) -> float:
    ...
    # 100 comes from the 100 Hz sampling rate of the ground truth data
...
# Trimming helpers
# ----------------------------
# used primarily in umeyama alignment to trim the starting seconds of the trajectory
...
# the files rosbagClock.py and qualisysClock.py give the times for this function
...
def umeyama_align(
    ...
    We restrict rotation to yaw because both measurement and simulation frames are right handed. Thus we need only
    align their starting positions and initial heading (yaw).

    Current implementation aligns on the first max_seconds of trajectories.
    """
    ...
    # this is because Umeyama assumes point clouds of equal length.
...
def main():
    ...
    gt_fit_smooth = truth_smooth.copy()
    ...
    # Plot 2: GT with CFD + Cuboid
    plot_three(f"Run {run_id}: Ground Truth with CFD and Cuboid", truth, cfd_aligned, cuboid_aligned)
    ...
    plot_three(f"Run {run_id}: GT with CFD and Cuboid (after 2 s cut)", truth_2s, cfd_2s_aligned, cuboid_2s_aligned)

    # Plot 4: Spike-smoothed GT with CFD + Cuboid
    cfd_smooth = apply_rotation_with_translation(cfd, align_R_cfd, truth_smooth[0])
    cuboid_smooth = apply_rotation_with_translation(cuboid, align_R_cuboid, truth_smooth[0])
    plot_three(f"Run {run_id}: Smoothed GT (Kalman+RTS-smoothed) with CFD and Cuboid", truth_smooth, cfd_smooth, cuboid_smooth)

    # Error metrics (raw GT vs aligned CFD/Cuboid)
    raw_stats = {
    ...
    plot_three(f"Run {run_id}: Smoothed GT (Kalman+RTS-smoothed) with CFD and Cuboid after 2 s cut", truth_smooth_2s, cfd_smooth_2s_aligned, cuboid_smooth_2s_aligned)

    # Error metrics (smoothed GT vs aligned CFD/Cuboid)
    smooth_stats = {
    ...
    plot_metrics_box("Runs 1-5: Error metrics (raw GT)", all_raw_stats)
    plot_metrics_box("Runs 1-5: Error metrics (Kalman+RTS-smoothed GT)", all_smooth_stats)
    plot_metrics_box("Runs 1-5: Error metrics (Kalman+RTS-smoothed GT after 2 s cut)", all_cut_stats)
