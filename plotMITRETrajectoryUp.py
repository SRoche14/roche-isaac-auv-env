#!/usr/bin/env python3
"""
Plot MITRE Up trajectories with Umeyama alignment.
...
Start times:
- startingUpTimes.json provides a start time (seconds) per run under keys
  qualyUp{i} and mitreUp{i}; these are used for trimming aligned trajectories.
...
- For /MITRE/Up, runs 2-6 correspond to Curee Test 12-16 by sorted order.
...
START_TIMES_JSON = BASE_DIR / "startingUpTimes.json"
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Plot MITRE Up trajectories with Umeyama alignment.")
    parser.add_argument(
        "--run-id",
        type=int,
