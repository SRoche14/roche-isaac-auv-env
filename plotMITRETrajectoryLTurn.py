#!/usr/bin/env python3
"""
Plot MITRE LTurn trajectories with yaw-only alignment.

Generated plots per run_id:
1) Ground truth vs odometry (aligned)
2) Ground truth with Cuboid and CFD (aligned)
3) Same as #2 after removing first 2 seconds (2 seconds total for LTurn)
4) Ground truth (Kalman+RTS-smoothed) with Cuboid and CFD (aligned)
5) Same as #4 after removing first 2 seconds (2 seconds total for LTurn)
...
Start times:
- startingLTurnTimes.json provides a start time (seconds) per run under keys
  qualyLTurn{i} and mitreLTurn{i}; these are used for trimming aligned trajectories.
- Ground truth sampling: 100 Hz.
- Odometry: 50 Hz.
- No additional pre-cut is applied after start-time trimming.
- CFD/Cuboid: 50 Hz (dt = 1/50).

Mapping notes:
- Run IDs (from bag filenames) are paired with Qualisys tests by sorted order.
- Tests {7, 12, 17, 21} are excluded before processing.
"""
...
START_TIMES_JSON = BASE_DIR / "startingLTurnTimes.json"
ALIGN_DISTANCE_M = 1.5
ALIGN_AXIS_MODE = "xy"
EXCLUDED_TEST_IDS = {7, 12, 17, 21}
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Plot MITRE LTurn trajectories with yaw-only alignment.")
    parser.add_argument(
        "--run-id",
        type=int,
    ...
    # Plot 0: Raw GT only
    plot_one(f"Run {run_id}: Ground Truth (raw)", truth)
    ...
    plot_three(f"Run {run_id}: GT with CFD and Cuboid (after 2 s total cut)", truth_2s, cfd_2s_aligned, cuboid_2s_aligned)
    ...
    # Plot 5: Spike-smoothed GT after 2 s total cut
    ...
    plot_three(f"Run {run_id}: Smoothed GT (Kalman+RTS-smoothed) with CFD and Cuboid after 2 s total cut", truth_smooth_2s, cfd_smooth_2s_aligned, cuboid_smooth_2s_aligned)
    ...
    plot_metrics_box(f"Runs {run_label}: Error metrics (Kalman+RTS-smoothed GT after 2 s total cut)", all_cut_stats)
